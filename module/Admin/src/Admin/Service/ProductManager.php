...
class ProductManager implements ServiceManagerAwareInterface
{
    ...
    public function editProduct(Product $product,$data){
        ...
        $category = $em->getRepository('\Admin\Entity\Category')->findOneBy(array('id'=> $product->getCategory()->getId()));
        $unit = $em->getRepository('\Admin\Entity\Unit')->findOneBy(array('id'=> $product->getUnit()->getId()));
